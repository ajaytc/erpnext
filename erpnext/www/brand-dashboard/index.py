...
def get_context(context):
    ...
    context.fabCount=frappe.db.sql("SELECT COUNT(name) FROM tabReminder WHERE seen='0' and order_type='fabric'")

    context.trimCount=frappe.db.sql("SELECT COUNT(name) FROM tabReminder WHERE seen='0' and order_type='trimming'")

    context.packCount=frappe.db.sql("SELECT COUNT(name) FROM tabReminder WHERE seen='0' and order_type='packaging'")
