...
def get_context(context):
    ...
    orders = frappe.get_list('Sales Order', filters={'company': brand, 'docstatus': 0}, fields=['name', 'customer'])
    
    context.order_items = {}
    for o in orders:
        context.order_items[o.name] = frappe.get_list('Sales Order Item',filters={'parent':o.name,'docstatus':0},fields=['name','item_code'])
    ...
    for order in order_items:
        for item in order_items[order]: 
            if item.item_code in temp_codes:
                temp_objects[item.item_code].append(order)
                continue
